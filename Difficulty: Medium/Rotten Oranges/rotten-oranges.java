    ...
    public int orangesRotting(int[][] mat) {
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < m; j++) {
                ...
                if (mat[i][j] == 2) {//add rotten oranges into queue and mark visited
                    vis[i][j] = 2;
        ...
        while (!q.isEmpty()) {
            ...
            for (int i = 0; i < 4; i++) {
                int nrow = r + drow[i];
                int ncol = c + dcol[i];
                if (nrow >= 0 && nrow < n && ncol >= 0 && ncol < m && vis[nrow][ncol] == 0 && mat[nrow][ncol] == 1) {//checking if fresh and not visited
